...
public class SourceSectionFilterTest {
    ...
    private static boolean isInstrumentedNode(SourceSectionFilter filter, Node instrumentedNode) {
        try {
            Method m = filter.getClass().getDeclaredMethod("isInstrumentedNode", Node.class, SourceSection.class);
            m.setAccessible(true);
            return (boolean) m.invoke(filter, instrumentedNode, instrumentedNode != null ? instrumentedNode.getSourceSection() : null);
        } catch (Exception e) {
    ...
    private static boolean isInstrumentedRoot(SourceSectionFilter filter, Node root) {
        try {
            Method m = filter.getClass().getDeclaredMethod("isInstrumentedRoot", SourceSection.class);
            m.setAccessible(true);
            return (boolean) m.invoke(filter, root != null ? root.getSourceSection() : null);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }

    private static Node createNode(final SourceSection section, final String... tags) {
        return new Node() {
    ...
    private static Node source(String... tags) {
